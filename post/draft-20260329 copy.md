## Blogs

### KZG vs IPA vs FRI: Picking the Right Polynomial Commitment Scheme
- A practical guide to the trade-offs between the three major polynomial commitment families powering modern zero-knowledge proof systems.
- <https://blog.zksecurity.xyz/posts/pcs-survey/>

### WTF Is a zkEVM and Why Should You Care
- <https://x.com/Snapcrackle/status/2036821613870461015?s=20>

### Unfaithful Claims: Breaking 6 zkVMs
- A zkVM verifier should be faithful to one thing above all else: its public claims. In this blog, they found six systems where this guarantee breaks. Learn how a subtle ordering bug lets an attacker bypass the cryptography entirely and prove mathematically impossible statements.
- <https://osec.io/blog/2026-03-03-zkvms-unfaithful-claims/>


### Post-quantum roundup: Week 13, 2026
- Neutral atom quantum computing, qubit types, open source in quantum, Ethereum's PQ new website
- <https://realmscape.substack.com/p/post-quantum-roundup-week-13-2026>


## Projects

### Ethereum Foundation’s New Post-Quantum Roadmap Website
- The Ethereum Foundation launched a new website detailing quantum vulnerabilities and the post-quantum migration roadmap for Ethereum, the second-most-popular blockchain.
- <https://pq.ethereum.org/>

## Videos

### Interval-Agnostic Vector Range Argument for Unstructured Range Assertions
- Hao Gao shared their recent work on Vector Range Arguments, to appear at Eurocrypt 2026
- paper link <https://eprint.iacr.org/2026/339>
- <https://youtu.be/faLRzx183As>

### Episode 396 - lean Ethereum Part 6: Formal Verification with Alex Hicks
- In this episode, Nico Mohnblatt speaks with Alex Hicks from the Ethereum Foundation about formal verification and its role in the lean Ethereum vision. This is the 6th and final episode of the lean Ethereum mini-series. Nico and Alex explore what it means to produce machine-checked proofs across the ZK stack, from RISC-V and zkVMs to circuits, compilers, and cryptographic primitives, and how these pieces connect in practice.
- <https://youtu.be/9u4fu7TiZCA?si=y3NaiicxP3K3EAJK>

### VOLEs and Non-interactive ZK proofs - Carsten Baum
- VOLE-in-the-head is a fast hash-based ZK proving system which is plausibly post-quantum secure. PSE invited Carsten Baum to give a talk on how it works, including how it was applied to build the FAEST signature scheme which is currently a NIST candidate, and the POMFRIT blind signature scheme.
- <https://youtu.be/5VjvkVPYdEg?si=wRYkMs4qSWK-YKpL>

## Papers

### CatCrypt: From Rust to Cryptographic Security in Lean
- Bas Spitters
- Recommend: 🌟🌟🌟🌟
- <https://eprint.iacr.org/2026/604>


### Post-Quantum Blockchains with Agility in Mind
- Manuel B. Santos, Danno Ferrin, Ron Kahat, Michael Lodder
- <https://eprint.iacr.org/2026/609>


### FrozenTRU: Cold Boot Attacks on NTRU-Based Hash-and-Sign Signatures
- Hiroto Kaihara, Mehdi Tibouchi, Masayuki Abe
- <https://eprint.iacr.org/2026/582>

### Bulletproofs*: Verifier-Efficient Arithmetic Circuit Proofs via Folding
- Emanuele Scala, Daniele Bartoli
- <https://eprint.iacr.org/2026/586>

### Speeding Up Sum-Check Proving (Extended Version)
-  Quang Dao, Zachary DeStefano, Suyash Bagad, Yuval Domb, Justin Thaler
- Recommend: 🌟🌟🌟🌟🌟
- <https://eprint.iacr.org/2026/587>


### FROSTLASS: Flexible Ring-Oriented Schnorr-like Thresholdized Linkably Anonymous Signature Scheme
- Joshua Babb, Brandon Goodell, Rigo Salazar, Freeman Slaughter, Luke Szramowski
- <https://eprint.iacr.org/2026/589>


### On the Security of Constraint-Friendly Map-to-Curve Relations
- Youssef El Housni, Benedikt Bünz
- <https://eprint.iacr.org/2026/590>

### Gryphes: Hybrid Proofs for Modular SNARKs with Applications to zkRollups
- Jiajun Xin, Samuel Cheung On Tin, Christodoulos Pappas, Yongjin Huang, Dimitrios Papadopoulos
- <https://eprint.iacr.org/2026/596>


### How Much Verifier's Dilemma and Staking Pools Adversely Affect Decentralization of Ethereum PoS under Realistic Operational Costs? (Extended Version)
- Ivan Homoliak, Martin Hruby, Martin Peresini, Kristian Kostal, Daria Smuseva
- <https://eprint.iacr.org/2026/578>

### vkproof: Succinct verification of indexed verifying keys using modular compilation and polynomial fingerprinting
- Antonio Mejias Gil, Xueqin Zhao
- <https://eprint.iacr.org/2026/581>


### Earpicks: Tightly Secure Two-Round Multi- and Threshold Signature
- Renas Bacho, Yanbo Chen 
- <https://eprint.iacr.org/2026/572>


### RoKoko: Lattice-based Succinct Arguments, a Committed Refinement
- Michael Klooss, Russell W. F. Lai, Ngoc Khanh Nguyen, Michał Osadnik, Lorenzo Tucci
- Recommend: 🌟🌟🌟
- <https://eprint.iacr.org/2026/575>


### Ticket to Hide: Private, Practical Proofs of Provenance for TLS
- Ryan Little, Daniel S. Roche, Mayank Varia
- <https://eprint.iacr.org/2026/540>

### Towards Verifiable AI with Lightweight Cryptographic Proofs of Inference
- Pranay Anchuri, Matteo Campanelli, Paul Cesaretti, Rosario Gennaro, Tushar M. Jois, Hasan S. Kayman, Tugce Ozdemir
- <https://eprint.iacr.org/2026/541>

### VERIDP: Verifiable Differentially Private Training
- Behzad Abdolmaleki, Amir R. Asadi, Vahid R. Asadi, Stefan Köpsell, Bhavish Mohee, Nahid Roustaeifar, Maryam Zarezadeh
- <https://eprint.iacr.org/2026/542>

### Aggregator-Based Voting using proof of Partition
- Marius Lombard-Platet, Doron Zarchy
- <https://eprint.iacr.org/2026/545>

### Succinct Verification of Lattice-Based Compressed Sigma-Protocols via Delegated Proofs of Correct Folding of Cryptographically Generated Public Parameters
- Anders Kallesøe
- <https://eprint.iacr.org/2026/551>

### NI-DKG: Non-Interactive Distributed Key Generation Using Blockchain and Zero-Knowledge Proofs
- Alex Kampa, Pau Escrich, Marta Bellés-Muñoz, Roger Baig
- <https://eprint.iacr.org/2026/552>

### New Approaches to Zero-Knowledge SNARG Constructions
- Chaya Ganesh, Mor Weiss
- Recommend: 🌟🌟
- <https://eprint.iacr.org/2026/562>

### Zeeperio: Verifying Governmental Elections with Ethereum
- Aikamdeep Malhotra, Aleksander Essex, Jeremy Clark
- <https://eprint.iacr.org/2026/565>

---
**If you’d like to receive updates via email, subscribe to us!**

- <https://paragraph.xyz/@zkinsights>